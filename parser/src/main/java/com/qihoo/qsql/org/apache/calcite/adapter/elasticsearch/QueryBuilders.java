...
class QueryBuilders {
  ...
  /**
   * Write usually simple (scalar) value (string, number, boolean or null) to json output. In case of complex objects
   * delegates to jackson serialization.
   *
  ...
  abstract static class QueryBuilder {
    ...
    //Modify by QSql team
    abstract void writeJson(JsonGenerator generator, List<String> fieldsList) throws IOException;

    //Modify by QSql team
    protected String transferFieldName(String fieldRef, List<String> fieldsList) {
      if (fieldRef.startsWith("$")) {
        int idx = Integer.parseInt(fieldRef.substring(1));
        return fieldsList.get(idx);
      } else {
        return fieldRef;
      }
    }
  ...
  static class BoolQueryBuilder extends QueryBuilder {
    ...
    //Modify by QSql team
    @Override
    void writeJson(JsonGenerator gen, List<String> fieldsList) throws IOException {
      gen.writeStartObject();
      gen.writeFieldName("bool");
      gen.writeStartObject();
      writeJsonArray("must", mustClauses, gen, fieldsList);
      writeJsonArray("filter", filterClauses, gen, fieldsList);
      writeJsonArray("must_not", mustNotClauses, gen, fieldsList);
      writeJsonArray("should", shouldClauses, gen, fieldsList);
      gen.writeEndObject();
      gen.writeEndObject();
    }
    ...
    //Modify by QSql team
    private void writeJsonArray(String field, List<QueryBuilder> clauses, JsonGenerator gen, List<String> fieldList)
        throws IOException {
      if (clauses.isEmpty()) {
        return;
      }
      ...
      if (clauses.size() == 1) {
        gen.writeFieldName(field);
        clauses.get(0).writeJson(gen, fieldList);
      } else {
        gen.writeArrayFieldStart(field);
        for (QueryBuilder clause : clauses) {
          clause.writeJson(gen, fieldList);
        }
        gen.writeEndArray();
      }
    }
  ...
  static class TermQueryBuilder extends QueryBuilder {
    ...
    //Modify By QSql team
    @Override
    void writeJson(JsonGenerator generator, List<String> fieldsList) throws IOException {
      generator.writeStartObject();
      generator.writeFieldName("term");
      generator.writeStartObject();
      generator.writeFieldName(transferFieldName(fieldName, fieldsList));
      writeObject(generator, value);
      generator.writeEndObject();
      generator.writeEndObject();
    }
  ...
  static class ExistsQueryBuilder extends QueryBuilder {
    ...
    void writeJson(JsonGenerator generator, List<String> fieldsList) throws IOException {
      ...
      generator.writeFieldName("exists");
      generator.writeStartObject();
      generator.writeStringField("field", transferFieldName(fieldName, fieldsList));
      generator.writeEndObject();
      generator.writeEndObject();
    }
  }
  ...
  /**
   * A query that wraps a filter and simply returns a constant score equal to the query boost for every document in
   * the filter.
   */
  static class ConstantScoreQueryBuilder extends QueryBuilder {
    ...
    void writeJson(JsonGenerator generator, List<String> fieldsList) throws IOException {
      ...
      generator.writeStartObject();
      generator.writeFieldName("filter");
      builder.writeJson(generator, fieldsList);
      generator.writeEndObject();
      generator.writeEndObject();
    }
  }
