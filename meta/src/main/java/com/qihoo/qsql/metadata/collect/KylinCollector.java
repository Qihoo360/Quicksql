...
public class KylinCollector extends MetadataCollector {
    ...
    protected List<String> getTableNameList() {
        ...
        try {
            DatabaseMetaData dbMetadata = connection.getMetaData();
            resultSet = dbMetadata.getTables(null, prop.getDbName().toUpperCase(), filterRegexp.toUpperCase(),
                    new String[] { "TABLE" });
            while (resultSet.next()) {
    ...
    private String getDatabasePosition() {
        ...
        try {
            ...
            while (resultSet.next()) {
                ...
                if (schema != null && schema.equalsIgnoreCase(prop.getDbName())) {
                    return schema;
                }
            }
        } catch (SQLException ex) {
