...
class EmptyScope implements SqlValidatorScope {
  ...
  //Modified by QSQL, for querying without db name.
  private void resolve_(final CalciteSchema rootSchema, List<String> names,
    ...
    for (String schemaName : concat) {
      ...
      if (concat.size() == 1) {
        ...
        NavigableMap<String, CalciteSchema> subSchemaMap = schema.getSubSchemaMap();
        Set<Map.Entry<String, CalciteSchema>> matchedSchemas = subSchemaMap.entrySet().stream()
        ...
        if (matchedSchemas.size() == 0) {
          throw new RuntimeException("Object '" + tableName + "' not found within any database");
        } else if (matchedSchemas.size() > 1) {
          throw new RuntimeException("Object '" + tableName + "' was found within more than one database," +
                  " please point specific database in front of '" + tableName + "'");
        } else {
          CalciteSchema matchedSchema = matchedSchemas.stream().findFirst().get().getValue();
          path = path.plus(null, -1, matchedSchema.name, StructKind.NONE);
