...
import com.qihoo.qsql.org.apache.calcite.sql.SqlDialectFactory;
...
public class ModelHandler {
    ...
    //Modified by QSQL
    public ModelHandler(SchemaPlus plus, String uri) throws IOException {
        this.rootSchemaPlus = plus;
        this.modelUri = uri;
        initialize(uri);
    }

    //Modified by QSQL
    private void initialize(String uri) throws IOException {
        JsonRoot root;
        ObjectMapper mapper;
     ...
     * @param upCase Whether to convert method names to upper case, so that they can be called
    ...
    public void visit(JsonRoot jsonRoot) {
        ...
        final Pair<String, SchemaPlus> pair =
            Pair.of(null, connection == null ? rootSchemaPlus : connection.getRootSchema());
        ...
        if (jsonRoot.defaultSchema != null) {
            try {
                if (connection != null) {
                    ...
                    defaultSchema = jsonRoot.defaultSchema;
                }
